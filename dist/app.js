...
function changeMap(oldMap) {
  var newMap = mapFactory(x, y);
  for (var i = 0; i < x / rate; i++) {
    for (var j = 0; j < y / rate; j++) {
      var neighbor = [(oldMap[i - 1] || [])[j - 1], (oldMap[i - 1] || [])[j], (oldMap[i - 1] || [])[j + 1], oldMap[i][j - 1], oldMap[i][j + 1], (oldMap[i + 1] || [])[j - 1], (oldMap[i + 1] || [])[j], (oldMap[i + 1] || [])[j + 1]].filter(function (x) {
        return x !== undefined;
